...
namespace EcommerceProject2.Controllers
{
    public class CartController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ChangeAmount(int orderCartId, int productId, string changeAmount)
        {
            using (var entities = new EcommerceDBEntities2())
            {
                var orderCart = entities.Order_Cart_Table.Find(orderCartId);
                ...
                if (orderCart != null)
                {
                    ...
                    else if (changeAmount == "decrease" && orderCart.Amount > 1)
                    {
                        orderCart.Amount--;
                    }

                    // Güncellenen miktarı ve toplam fiyatı hesapla
                    decimal? totalPrice = CalculateTotalPrice(orderCart.Customer_id);
                    ViewBag.TotalPrice = totalPrice;

                    // Güncellenen miktarı kaydet
                    entities.SaveChanges();
                }

                // Miktar güncellendikten sonra tekrar sepettekileri göster
                return RedirectToAction("GetOrderCartItems");
            }
        }
